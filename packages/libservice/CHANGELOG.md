...
- Fixed `Actor.loadProto()` path resolution bug where it incorrectly relied on
  ...
  upward from both module directory and CWD for `generated/proto` and supports
  explicit override via `SERVICE_PROTO_DIR` or `PROTO_ROOT`.
- Added internal helper `#loadFromFile()` for clearer separation of discovery
  vs loading logic.

## 2025-09-02
## 2025-09-05

- Updated `Actor.loadProto()` to load schemas exclusively from
  `generated/proto` (removed previous dynamic / storage-based lookup logic)
