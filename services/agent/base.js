...
import { AgentRequestSchema } from "../../generated/agent_pb.js";
...
export class AgentBase {
  ...
  /**
   * Creates a new Agent service base
  ...
  /**
   * Load proto definition for agent service
   * @returns {Promise<object>} Proto definition
   */
  async loadProto() {
    const storage = storageFactory("proto", "local");
    const filename = await storage.path("agent.proto");
    ...
    return packageDefinition.agent;
  }
  ...
  async start() {
    ...
    const handlers = {};
    handlers.ProcessRequest = this.#createHandler(
      "ProcessRequest",
      AgentRequestSchema,
    );
   ...
   * @param {object} _request - AgentRequest message (typed using @bufbuild/protobuf)
   * @param {object} _metadata - gRPC metadata
   * @param {object} _call - gRPC call object
   * @returns {Promise<object>} AgentResponse response
   */
  async ProcessRequest(_request, _metadata, _call) {
