...
import { EmbeddingRequestSchema } from "../../generated/llm_pb.js";
import { CompletionRequestSchema } from "../../generated/llm_pb.js";
...
export class LlmBase {
  ...
  /**
   * Creates a new Llm service base
  ...
  /**
   * Load proto definition for llm service
   * @returns {Promise<object>} Proto definition
   */
  async loadProto() {
    const storage = storageFactory("proto", "local");
    const filename = await storage.path("llm.proto");
  ...
  async start() {
    ...
    const handlers = {};
    handlers.CreateEmbeddings = this.#createHandler(
      "CreateEmbeddings",
      EmbeddingRequestSchema,
    );
    handlers.CreateCompletions = this.#createHandler(
      "CreateCompletions",
   ...
   * @param {object} _request - EmbeddingRequest message (typed using @bufbuild/protobuf)
   * @param {object} _metadata - gRPC metadata
   * @param {object} _call - gRPC call object
   * @returns {Promise<object>} EmbeddingResponse response
   */
  async CreateEmbeddings(_request, _metadata, _call) {
    throw new Error("CreateEmbeddings method must be implemented by subclass");
  }

  /**
   * CreateCompletions RPC method
   * @param {object} _request - CompletionRequest message (typed using @bufbuild/protobuf)
   * @param {object} _metadata - gRPC metadata
   * @param {object} _call - gRPC call object
   * @returns {Promise<object>} CompletionResponse response
   */
  async CreateCompletions(_request, _metadata, _call) {
    throw new Error("CreateCompletions method must be implemented by subclass");
  }
}
