...
- Updated default `model` from `claude-3.5-sonnet` to `gpt-4o` in `config/config.yml`, `config/config.example.yml`, and `services/llm/server.js`
- Fixed tool name derivation using `id.name.split('.')` ensuring plain function names are sent to provider for function calling compatibility
