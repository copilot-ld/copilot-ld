...
class LlmService extends LlmBase {
  ...
  async CreateCompletions(req) {
    ...
    const messages = req.messages.map((m, idx) => {
      ...
      if (m.role === 'assistant' && Array.isArray(m.tool_calls) && m.tool_calls.length > 0) {
        ...
        out.tool_calls = m.tool_calls.map(tc => ({
          ...
          function: {
            name: (tc.function?.id?.name && tc.function.id.name.split('.').pop()) || tc.function?.name || 'unknown',
    ...
    const tools = req.tools?.map((t) => {
      ...
      const simpleName = rawIdName.includes('.') ? rawIdName.split('.').pop() : rawIdName;
      const tool = {
        type: t.type,
        function: {
          name: simpleName || t.function.name || 'unknown',
          description: t.function.descriptor?.purpose || String(t.function.descriptor) || '',
          parameters: t.function.parameters || {},
        },
      };
      
      // Ensure parameters is a plain object, not a complex type
      if (tool.function.parameters && typeof tool.function.parameters.toObject === 'function') {
      ...
      if (tool.function.parameters && tool.function.parameters.properties) {
        if (!tool.function.parameters.required) {
          ...
          // For other tools, make all properties required by default
            ...
            tool.function.parameters.required = Object.keys(tool.function.parameters.properties);
          }
        }
        
        // Improve parameter descriptions
        if (tool.function.parameters.properties.input) {
          tool.function.parameters.properties.input.description = 'The text input to process';
        }
      }
    ...
    try {
      ...
      this.debug("CreateCompletions failed, retrying without tools", { error: err.message });
      // Detect zero-limit condition and permanently drop tools for this attempt
      const zeroLimit = /limit of 0/.test(err.message);
      data = await llm.createCompletions({ messages, temperature: req.temperature });
      // Attach a synthetic assistant note so downstream can explain lack of tools
      data.choices = data.choices || [];
      data.choices.push({
        index: data.choices.length,
        message: common.MessageV2.fromObject({ role: 'system', content: zeroLimit ? 'Provider reported zero prompt token limit for tools; responded without tool calling.' : 'Tool execution unavailable: falling back to direct response.' }),
        finish_reason: 'stop'
      });
    }
