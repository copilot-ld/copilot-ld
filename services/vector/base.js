...
import { QueryItemsRequestSchema } from "../../generated/vector_pb.js";
...
export class VectorBase {
  ...
  /**
   * Creates a new Vector service base
  ...
  /**
   * Load proto definition for vector service
   * @returns {Promise<object>} Proto definition
   */
  async loadProto() {
    const storage = storageFactory("proto", "local");
    const filename = await storage.path("vector.proto");
    ...
    return packageDefinition.vector;
  }
  ...
  async start() {
    ...
    const handlers = {};
    handlers.QueryItems = this.#createHandler(
      "QueryItems",
      QueryItemsRequestSchema,
    );
   ...
   * @param {object} _request - QueryItemsRequest message (typed using @bufbuild/protobuf)
   * @param {object} _metadata - gRPC metadata
   * @param {object} _call - gRPC call object
   * @returns {Promise<object>} QueryItemsResponse response
   */
  async QueryItems(_request, _metadata, _call) {
    throw new Error("QueryItems method must be implemented by subclass");
  }
}
