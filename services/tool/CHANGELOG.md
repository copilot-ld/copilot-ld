...
- Added enhanced diagnostic logging for `ExecuteTool` including structural key dump and resolved name trace
- Implemented redundant `function.name` to aid downstream name extraction
- Added multi-stage tool name resolution with id synthesis when only `function.name` present
- Implemented safe JSON argument parsing with error capture instead of throw
- Added explicit log for resolved tool name to distinguish logging mismatch vs actual absence
...
- Fixed `ToolClient.ExecuteTool` to reference `common.ToolCallResult` instead of non-existent `tool.ToolCallResult`, preventing runtime `fromObject` errors
