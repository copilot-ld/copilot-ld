...
describe("Server", () => {
  ...
  test("should call service methods during setup", async () => {
    ...
    // Mock storage factory to avoid file system access
    const originalStorageFactory = global.storageFactory;
    global.storageFactory = mock.fn(() => ({
      path: mock.fn(async () => "/mock/path/test.proto"),
    }));
    ...
    try {
      ...
      global.storageFactory = originalStorageFactory;
    }
  });
});
