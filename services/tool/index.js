  ...
  * @param {object} req - Tool execution request (raw object due to generation gap)
  ...
  async ExecuteTool(req) {
    ...
    try {
      ...
      let rawName = (req.function && req.function.id && req.function.id.name) || req.function?.name || '';
      ...
      if (!req.function?.id?.name && req.function?.name) {
        req.function.id = req.function.id || { type: 'common.ToolFunction', parent: '' };
        req.function.id.name = `common.ToolFunction.${req.function.name}`;
        rawName = req.function.id.name;
      }
      if (rawName && !rawName.includes('.')) rawName = `common.ToolFunction.${rawName}`;
      ...
      const result = await this.#routeToService(packageName, serviceName, methodName, req);
  ...
  async #createServiceClient(packageName, serviceName) {
    ...
    switch (`${packageName}.${serviceName}`) {
      ...
      case "hash.Hash": {
        const { HashClient } = await import("../../generated/tools/hash/client.js");
   ...
   * @private
   */
  async #convertToolToServiceRequest(packageName, serviceName, methodName, toolRequest) {
    // Parse tool arguments
    const argsRaw = toolRequest.function?.arguments || toolRequest.function?.parameters?.arguments;
    let args = {};
    if (argsRaw) {
      try {
        args = JSON.parse(argsRaw);
      } catch (e) {
        this.debug('Arguments JSON parse failed', { error: e.message, argsSample: argsRaw.slice(0,120) });
