...
export class HistoryBase {
  ...
  /**
   * Creates a new History service base
  ...
  /**
   * Load proto definition for history service
   * @returns {Promise<object>} Proto definition
   */
  async loadProto() {
    const storage = storageFactory("proto", "local");
    const filename = await storage.path("history.proto");
    ...
    return packageDefinition.history;
  }
   ...
   */
  async GetHistory(_request, _metadata, _call) {
    throw new Error("GetHistory method must be implemented by subclass");
  }
   ...
   */
  async UpdateHistory(_request, _metadata, _call) {
    throw new Error("UpdateHistory method must be implemented by subclass");
  }
}
