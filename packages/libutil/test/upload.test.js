...
describe("Upload", () => {
  ...
  test("constructor validates required dependencies", () => {
    assert.throws(() => new Upload(null, mockLogger), /storageFn is required/);

    assert.throws(
      () => new Upload(mockStorageFactory, null),
      /logger is required/,
    );
  });
  ...
  test("filters hidden files starting with dot during upload", async () => {
    ...
    const upload = new Upload(mockStorageFactory, mockLogger, ["test"]);
