...
transform-pdf:  ## Transform documents (PDF)
	@$(ENVLOAD) npx --workspace=@copilot-ld/libtransform transform-pdf
...
ingest-load:  ## Load documents into pipeline
	@$(ENVLOAD) npx --workspace=@copilot-ld/libingest ingest-load
...
process-resources:  ## Process knowledge resources
	@$(ENVLOAD) npx --workspace=@copilot-ld/libresource process-resources

.PHONY: process-tools
process-tools:  ## Process tool definitions
	@$(ENVLOAD) node ./scripts/tools.js

.PHONY: process-vectors
process-vectors:  ## Process vector indices
	@$(ENVLOAD) npx --workspace=@copilot-ld/libvector process-vectors

.PHONY: process-graphs
process-graphs:  ## Process graph indices
	@$(ENVLOAD) npx --workspace=@copilot-ld/libgraph process-graphs
...
rc-stop:  ## Stop services via rc
	@$(ENVLOAD) node packages/librc/bin/rc.js stop

.PHONY: rc-restart
rc-restart:  ## Restart services via rc
	@$(ENVLOAD) node packages/librc/bin/rc.js restart
...
storage-wait:  ## Wait for storage to be ready
	@$(ENVLOAD) npx --workspace=@copilot-ld/libstorage storage wait

.PHONY: storage-init
storage-init:  ## Create bucket in storage backend
	@$(ENVLOAD) npx --workspace=@copilot-ld/libstorage storage create-bucket

.PHONY: storage-upload
storage-upload:  ## Upload data to storage backend
	@$(ENVLOAD) npx --workspace=@copilot-ld/libstorage storage upload

.PHONY: storage-download
storage-download:  ## Download data from storage backend
	@$(ENVLOAD) npx --workspace=@copilot-ld/libstorage storage download

.PHONY: storage-list
storage-list:  ## List storage contents
	@$(ENVLOAD) npx --workspace=@copilot-ld/libstorage storage list
...
eval-report:  ## Generate evaluation report
	@$(ENVLOAD) npx --workspace=@copilot-ld/libeval eval-report
...
download-bundle:  ## Download generated code bundle from S3
	@$(ENVLOAD) npx --workspace=@copilot-ld/libutil download-bundle
