...
### Package Organization for Processors and Indices

**Processor Organization**: All processor implementations must be organized in a
`processor/` subdirectory within their package with descriptive filenames:

```
packages/libresource/processor/resource.js  # ResourceProcessor
packages/libvector/processor/vector.js      # VectorProcessor
packages/libgraph/processor/graph.js        # GraphProcessor
packages/libgraph/processor/ontology.js     # OntologyProcessor
packages/libagent/processor/agent.js        # AgentProcessor
```

**Index Organization**: All specialized index implementations must be organized
in an `index/` subdirectory within their package with descriptive filenames:

```
packages/libtelemetry/index/trace.js        # TraceIndex
packages/libmemory/index/memory.js          # MemoryIndex
packages/libgraph/index/graph.js            # GraphIndex
packages/libvector/index/vector.js          # VectorIndex
```

**Critical Import Requirements**:

1. **Processors are NEVER exported** from main package `index.js` files to avoid
   pulling in unnecessary dependencies during runtime operations
2. **Indices are NEVER exported** from main package `index.js` files to avoid
   circular dependencies with generated code (indices depend on
   `@copilot-ld/libtype`)
3. **Always use explicit imports** for processors and indices from their
   dedicated files:

```javascript
// ✅ CORRECT - Direct imports from dedicated files
...
```javascript
// ❌ INCORRECT - Importing from main package index
import { VectorProcessor, VectorIndex } from "@copilot-ld/libvector";
import { TraceIndex } from "@copilot-ld/libtelemetry";
```

**Rationale**:

- **Processors** are only used during development/build time (knowledge base
  processing, data preparation) and should not be loaded during runtime service
  operations
- **Indices** depend on generated types from `@copilot-ld/libtype` which creates
  circular dependencies during code generation if exported through main package
  files
- **Explicit imports** ensure clear dependency chains and prevent accidental
  loading of heavy dependencies
...
11. **DO NOT** export processors from main package `index.js` files - always
    import directly from `processor/` subdirectories to avoid loading
    unnecessary dependencies
12. **DO NOT** export indices from main package `index.js` files - always import
    directly from `index/` subdirectories to avoid circular dependencies with
    generated code
...
- Instead of exporting processors from main index → Import directly from
  `processor/` subdirectories to avoid runtime overhead
- Instead of exporting indices from main index → Import directly from `index/`
  subdirectories to prevent circular dependencies
